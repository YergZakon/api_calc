...
0. Штраф (`FS1R64=05`)
1. Исправительные работы (`FS1R64=06`)
2. Обязательные работы (`FS1R64=09`)
3. Ограничение свободы (`FS1R64=11`)
4. Арест (`FS1R64=12`)
5. Лишение свободы (`FS1R64=01`)
6. Смертная казнь (`FS1R64=02`)
7. Лишение звания/класса (`FS1R65=01`)
8. Запрет занимать должность (`FS1R65=04`)
9. Запрет заниматься деятельностью (`FS1R65=22`)
10. Лишение права (срок) (`FS1R65=02`)
11. Конфискация имущества (`FS1R65=05`)
12. Резерв (не используется)
13. Резерв (не используется)
14. Метаданные (проступок / отсутствие УО)

#### `aNakaz` — колонки (индексы)
0. `is_applicable` — применимо (bool)
1. `min` — минимальное значение (число)
2. `max` — максимальное значение (число)
3. `formatted_text` — строка (человекочитаемо)
4. `min_years` — для индексов 3 и 5
5. `min_months` — для индексов 3 и 5
6. `min_days` — для индексов 3 и 5
7. `max_years` — для индексов 3 и 5
8. `max_months` — для индексов 3 и 5
9. `max_days` — для индексов 3 и 5
10. внутренний код причины/ограничения (int)
11. служебный счётчик (используется для лишения свободы)
12. внутренний код ветки расчёта (int)
